...
const redactNode = (node: Record<string, unknown>): void => {
  if ('text' in node) node.text = redacted
...
const redactUpdates = (method: string, params: unknown[]): void => {
  ...
  if (method === 'Viewlet.setProperty' && ['value', 'textContent'].includes(params[2] as string)) params[3] = redacted
  ...
  for (const patch of updates) {
    if (patch.type === 1 || ['value', 'textContent'].includes(patch.key)) patch.value = redacted
  }
}
